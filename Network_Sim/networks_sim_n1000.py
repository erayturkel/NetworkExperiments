...
def knapsack_greedy(adj, cov_mat, cost_max):
    ...
    for i in range(n):
        ...
        var_reduction=1/np.trace(np.linalg.inv(np.dot(covs_node,covs_node.T)))
        values[i]=var_reduction/np.sum(adj[i,])
...
def learn_gamma(adj,cov_mat,lambda_vec,seq):
    ...
    inv_component=np.linalg.inv(np.diagflat(adj.dot(np.ones(n))))
    total_outcomes=np.zeros(n)
    total_covs=np.zeros((k,n))
    total_exps=np.zeros(n)
    already_exposed=np.zeros(n)
    l=0
    for node in seq:
        treated=np.zeros(n)
        treated[node]=1
        expose=np.dot(treated,adj)
        expose=np.maximum(expose,already_exposed)-already_exposed
        already_exposed= np.maximum(expose,already_exposed)
        exp_exposed=np.dot(inv_component,expose.T)
        exp_exposed=exp_exposed[np.array(expose,dtype=bool)[0]]
        covs_exposed=cov_mat[:,np.array(expose,dtype=bool)[0]]
...
def simul_numbers(num_sim,graph_size,graph_connect,cov_num,max_deploy,max_learn):
    ...
    for i in range(num_sim):
        ...
        mat_rand_graph=mat_rand_graph.tolil()
...
def simul_grid(graph_size,graph_connect,num_sims):
    max_deploy_grid=np.linspace(start=50, stop=50, num=1)
    cov_num=5
    mean_gap_grid=np.zeros(shape=(1,8))
    sdev_gap_grid=np.zeros(shape=(1,8))
    i=0
    for j in max_deploy_grid:
        k=0
        max_learn_grid=np.linspace(start=10, stop=100, num =8)
...
result_sims=simul_grid(100,0.1,1000)
np.savetxt('sim_results_n100_p_01_c_5_grid_mean.csv', result_sims['mean_gaps'], delimiter=',')
np.savetxt('sim_results_n100_p_01_c_5_grid_sdev.csv', result_sims['sdev_gaps'], delimiter=',')
